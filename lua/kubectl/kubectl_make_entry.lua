...
function k_make_entry.gen_from_object_with_status(_)
	local displayer = entry_display.create({
		separator = " ",
		items = {
			{ width = 4 },
...
function k_make_entry.gen_from_object(resource)
	...
	local make_display = function(entry)
		...
		local e = {
			vim.iter({ time, status and status.hl }):flatten(math.huge):totable(),
		}
		if status then
			table.insert(e, vim.iter({ status.status, status.hl }):flatten(math.huge):totable())
		end
		if ns then
			table.insert(e, vim.iter({ ns, status and status.hl }):flatten(math.huge):totable())
		end
		table.insert(
			e,
			vim.iter({ entry.value.metadata.name, status and status.hl }):flatten(math.huge):totable()
				or entry.value.metadata.name
		)
		return displayer(e)
	end
...
function k_make_entry.gen_for_port_forward(_)
	local displayer = entry_display.create({
		separator = " ",
		items = {
			{ width = 10 },
...
function k_make_entry.gen_for_api_resources(_)
	local displayer = entry_display.create({
		separator = " ",
		items = {
			{ remaining = true },
