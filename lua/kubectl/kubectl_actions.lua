...
function M.pod_log(pod, opts)
  return function(prompt_bufnr)
    ...
    vim.api.nvim_buf_set_option(0, "filetype", opts.filetype or "log")
...
function M.describe_resource(opts)
  return function(prompt_bufnr)
    ...
    vim.api.nvim_buf_set_option(0, "buftype", "nofile")
    vim.api.nvim_buf_set_option(0, "swapfile", false)
    vim.api.nvim_buf_set_option(0, "filetype", "txt")
    vim.api.nvim_buf_set_option(0, "bufhidden", "wipe")
...
function M.delete_resource(opts)
  return function(prompt_bufnr)
    ...
    local confirm = vim.fn.input("Delete " .. selection.value.kind .. "/" .. selection.value.metadata.name .. "? [y/N] ")
    if confirm ~= "y" then
      return
    end
